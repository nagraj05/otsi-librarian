...
export default async function LeaderboardPage() {
  ...
  const [rawUsers, role] = await Promise.all([getLeaderboard(), getAdminRole(userId)]);
  ...
  return (
    <div className="min-h-screen">
      <Navbar isAdmin={role === 'admin'} activePath="leaderboard" />

      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-9 h-9 rounded-xl bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center">
            <Trophy className="w-4.5 h-4.5 text-amber-500" style={{ width: 18, height: 18 }} />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-foreground tracking-tight">Leaderboard</h2>
            <p className="text-muted-foreground text-sm">Reading streaks across the team.</p>
          </div>
        </div>
            ...
            {users.map((u, i) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-4 rounded-2xl ring-1 shadow-sm p-4 transition-all duration-200 hover:shadow-md group ${
                    isMe
                ...
                >
                  ...
                  {/* Avatar */}
                  <div className={`w-10 h-10 rounded-2xl ${avatarColor(u.name)} flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-sm`}>
                    {initials(u.name)}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`font-semibold text-sm truncate ${isMe ? 'text-brand' : 'text-foreground'}`}>
                        {u.name}
                      </p>
                      {isMe && (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-brand text-white shrink-0">you</span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <Flame className={`w-4 h-4 ${
                      u.streak === 0      ? 'text-muted-foreground/30' :
                      isTop               ? 'text-orange-500' :
                                            'text-orange-400'
                    }`} />
                    <span className={`text-lg font-bold leading-none ${
                      u.streak === 0 ? 'text-muted-foreground/40' :
                      isTop          ? 'text-orange-500' :
                                       'text-foreground'
                    }`}>
                      {u.streak}
